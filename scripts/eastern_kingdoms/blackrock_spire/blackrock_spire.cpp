...
bool GOUse_go_father_flame(Player* pPlayer, GameObject* pGo)
{
    if (instance_blackrock_spire* m_pInstance = (instance_blackrock_spire*)pGo->GetInstanceData())
    {
        ...
        m_pInstance->GetData(TYPE_DRAKKISATH) == DONE) // when Drakkisath is dead, event don't start
            return false;
...
bool GOUse_go_challenge_to_urok(Player* pPlayer, GameObject* pGo)
{
    ...
    if (Creature* pAdd = GetClosestCreatureWithEntry(pGo, NPC_UROK_ENFORCER | NPC_UROK_OGRE_MAGUS, 20.f))
    {
        if (pAdd->getVictim())
...
{
    mob_blackhand_dragon_handlerAI(Creature* pCreature) : ScriptedAI(pCreature) {Reset();}
