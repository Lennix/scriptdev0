...
void instance_blackrock_depths::DoUseFlamelashRunes()
{
    if (!m_uiFlamelashRunes.empty())
    {
        for(GUIDList::iterator itr = m_uiFlamelashRunes.begin(); itr != m_uiFlamelashRunes.end(); ++itr)
            if (GameObject* GoRune = instance->GetGameObject(*itr))
        ...
        case TYPE_TOMB_OF_SEVEN:
            ...
            switch(uiData)
            {
                case IN_PROGRESS:
                    ...
                    break;
                case FAIL:
                    if (m_auiEncounter[7] == IN_PROGRESS)       //prevent use more than one time
                        DoUseDoorOrButton(GO_TOMB_ENTER);
                    break;
                case DONE:
                    DoRespawnGameObject(GO_CHEST_SEVEN, 120*MINUTE);
...
uint32 instance_blackrock_depths::GetData(uint32 uiType)
{
    ...
    {
        case TYPE_RING_OF_LAW:
            return m_auiEncounter[0] == IN_PROGRESS && m_bBannerSpawned ? SPECIAL : m_auiEncounter[0];
        case TYPE_VAULT:
            return m_auiEncounter[1] == IN_PROGRESS && m_uiCoffersOpenedCount == 12 ? SPECIAL : m_auiEncounter[1];
        case TYPE_BAR:
        ...
        case TYPE_BAR4:
            return ((m_auiEncounter[5] == IN_PROGRESS && m_uiBarrelsDestroyedCount == 3) ? SPECIAL : m_auiEncounter[5]);
        case TYPE_BAR5:
