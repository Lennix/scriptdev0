...
struct MANGOS_DLL_DECL boss_atalalarionAI : public ScriptedAI
{
    ...
    {
        if (m_pInstance)
            m_pInstance->SetData(TYPE_ATALALARION, DONE);
    }
...
bool GossipHello_go_altar_of_hakkar(Player* pPlayer, GameObject* pGo)
{
    ...
    if (m_pInstance && m_pInstance->GetData(TYPE_ATALALARION) == IN_PROGRESS)
...
bool GossipSelect_go_altar_of_hakkar(Player* pPlayer, GameObject* pGo, uint32 uiSender, uint32 uiAction)
{
    ...
    {
        m_pInstance->SetData(TYPE_ATALALARION, NOT_STARTED);
