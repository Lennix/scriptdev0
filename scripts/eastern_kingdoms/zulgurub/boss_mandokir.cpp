...
struct MANGOS_DLL_DECL boss_mandokirAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        m_uiChargeTimer = 90000;    // <--- this time probably wrong
    ...
    Player* GetMaxAlonePlayer()
    {
        ...
        float  m_fMaxAlonePlayerDist  = NULL;
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (m_uiWatchTimer <= uiDiff)
        {
            ...
            if (m_uiWatchTargetGUID)
            {
                ...
                if (pWatchTarget && m_creature->getThreatManager().getThreat(pWatchTarget) > m_fTargetThreat)
                    DoActThreatingGaze(pWatchTarget);
                else
                {
                    m_fTargetThreat = NULL;
                    m_uiWatchTargetGUID.Clear();
                }
            }
            else
