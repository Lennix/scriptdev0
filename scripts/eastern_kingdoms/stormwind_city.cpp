...
struct MANGOS_DLL_DECL npc_lord_gregor_lescovarAI : public npc_escortAI
{
    ...
    bool GetCreaturesForQuest(float fMaxSearchRange = 10.0f)
    {
        ...
        else
        {
            ...
            {
                debug_log("SD0: GetClosestCreatureWithEntry() failed! Entry of returned unit != entry of searched unit!");
                return false;
            }
        }
        ...
        if (!Marzon)
        {
            debug_log("SD0: Creature 'Marzon The Silent Blade' (entry %u) has been not found for quest 'The Attack!'", NPC_MARZON_THE_SILENT_BLADE);
            return false;
        }
        else
        {
            ...
            if (m_uiEventTimer <= uiDiff)
            {
                ...
                switch(m_uiEventPhase)
                {
                    case 10:
                        ...
                        if (Creature* pWindsorsHorse = m_creature->SummonCreature(NPC_WINDSOR_HORSE, x+1, y+2, z, m_creature->GetOrientation(), TEMPSUMMON_DEAD_DESPAWN, NULL))
                        {
                    ...
                    case 12:
                        if (Creature* pWindsorsHorse = m_creature->GetMap()->GetCreature(m_uiWindsorsHorseGUID))
                        {
                            ...
                            pWindsorsHorse->GetMotionMaster()->MovePoint(NULL, -9104.79f, 385.41f, 93.51f);
                        }
                        m_uiEventTimer = 2000;
                        break;
                    case 13:
                    ...
                    case 39:
                        ...
                        if (MarcusGuard[1])
                            MarcusGuard[1]->GetMotionMaster()->MovePoint(NULL, -8960.99f, 503.33f, 96.59f);
                        if (MarcusGuard[2])
                            MarcusGuard[2]->GetMotionMaster()->MovePoint(NULL, -8958.96f, 504.91f, 96.59f);
                        m_uiEventTimer = 4500;
                        break;
                    case 40:
                    ...
                    case 43:
                        ...
                        if (MarcusGuard[4])
                        {
                            MarcusGuard[4]->GetMotionMaster()->MovePoint(NULL, -8972.82f, 518.37f, 96.59f);
                        }
                        if (MarcusGuard[3])
                        {
                            MarcusGuard[3]->GetMotionMaster()->MovePoint(NULL, -8970.77f, 520.09f, 96.59f);
                        }
                        m_uiEventTimer = 4000;
                        break;
                    case 44:
                    ...
                    case 48:
                        ...
                        if (pMarcus)
                            pMarcus->GetMotionMaster()->MovePoint(NULL, -8977.37f, 510.82f, 96.60f);
                        m_uiEventTimer = 3000;
                        break;
                    case 49:
                        if (pMarcus)
                    ...
                    case 70:
                        if (pLadyKatrana)
                            m_creature->SetFacingToObject(pLadyKatrana);
                    ...
                    case 93:
                        if (pAnduinWrynn)
                        {
                            ...
                            pAnduinWrynn->GetMotionMaster()->MovePoint(NULL, -8505.08f, 338.17f, 120.88f);
                        }
                        m_uiEventTimer = 2000;
                        break;
                    case 94:
                    ...
                    case 103:
                        if (pBolvar)
                        {
                            ...
                            pBolvar->GetMotionMaster()->MovePoint(NULL, -8450.43f, 336.85f, 121.32f);
                        }
                        m_uiEventTimer = 1000;
                        break;
                    case 104:
                            ...
                            if (!m_uiSwRoyalGuardList.empty())
                            {
                                for(std::list<Creature*>::const_iterator itr = m_uiSwRoyalGuardList.begin(); itr != m_uiSwRoyalGuardList.end(); ++itr)
                                    if ((*itr))
                                    {
                                        ...
                                        if (Creature* pLadyOnyxiaSpawn = m_creature->SummonCreature(NPC_ONYXIAS_ELITE_GUARD, x, y, (z + 1.0f), (*itr)->GetOrientation(), TEMPSUMMON_DEAD_DESPAWN, NULL))
                                        {
                    ...
                    case 108:
                        ...
                        break;
                    case 109:
                        // Respawn guards in throne hall to help in the fight
                    ...
                    case 113:
                        if (pBolvar)
                        {
                            ...
                            pBolvar->GetMotionMaster()->MovePoint(NULL, x-1.0f, y-1.0f, z);
                        }
                        m_uiEventTimer = 2000;
                        break;
                    case 114:
                        if (pBolvar && !pBolvar->IsWithinDist(m_creature, 4.0f))
                        {
                            ...
                            if (pBolvar->HasSplineFlag(SPLINEFLAG_WALKMODE))
                                pBolvar->RemoveSplineFlag(SPLINEFLAG_WALKMODE);
                            pBolvar->GetMotionMaster()->MovementExpired();
                            pBolvar->GetMotionMaster()->MovePoint(NULL, x-2, y-2, z);
                            --m_uiEventPhase;
                        }
                    ...
                    case 121:
                        ...
                        if (pLadyKatrana)
                        {
                            pLadyKatrana->SetDisplayId(pLadyKatrana->GetCreatureInfo()->ModelId[0]);
