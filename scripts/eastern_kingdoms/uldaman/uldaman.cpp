...
bool AT_map_chamber(Player* pPlayer, AreaTriggerEntry const* pAt)
{
...
bool GOUse_go_keystone_chamber(Player *pPlayer, GameObject* pGo)
{
    if (ScriptedInstance* m_pInstance = (ScriptedInstance*)pGo->GetInstanceData())
        m_pInstance->SetData(TYPE_IRONAYA, IN_PROGRESS); //door animation and save state.
...
bool GOUse_go_shadowforge_cache(Player* pPlayer, GameObject* pGo)
{
    if (pGo->GetUseCount() == 0)
    {
        ...
        for(uint8 itr = 0; itr < 2; ++itr)
            pPlayer->SummonCreature(NPC_SHADOWFORGE_AMBUSHER, x + irand(-3,3), y + irand(-3,3), z, 0.0f, TEMPSUMMON_DEAD_DESPAWN, 0);
    }
    return false;
}
...
void AddSC_uldaman()
{
    ...
    pNewScript->Name="at_map_chamber";
    pNewScript->pAreaTrigger = &AT_map_chamber;
    pNewScript->RegisterSelf();

    pNewScript = new Script;
    pNewScript->Name="go_keystone_chamber";
    pNewScript->pGOUse = &GOUse_go_keystone_chamber;
    pNewScript->RegisterSelf();

    pNewScript = new Script;
    pNewScript->Name="go_shadowforge_cache";
    pNewScript->pGOUse = &GOUse_go_shadowforge_cache;
    pNewScript->RegisterSelf();
