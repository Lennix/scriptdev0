...
enum Spells
{
    ...
    SPELL_JUDGEMENT_OF_JUSTICE          = 20184,        // Rank 1 -> Malicious
    SPELL_JUDGEMENT_OF_LIGHT            = 20346,        // Rank 4 -> Banality
    SPELL_JUDGEMENT_OF_WISDOM           = 20355,        // Rank 3 -> Shadowed
    SPELL_JUDGEMENT_OF_RIGHTEOUSNESS    = 20286,        // Rank 8 -> Corrupted
...
struct MANGOS_DLL_DECL boss_death_knight_darkreaverAI : public ScriptedAI
{
    ...
    void JustDied(Unit* pKiller)
    {
        if (m_pInstance)
        {
            if (pKiller->GetTypeId() == TYPEID_PLAYER && ((Player*)pKiller)->GetTeam() == ALLIANCE)
...
struct MANGOS_DLL_DECL mob_aspectAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        switch(m_creature->GetEntry())
        {
            case NPC_ASPECT_OF_BANALITY:
                m_uiJudgementEntry = SPELL_JUDGEMENT_OF_LIGHT;
            ...
            case NPC_ASPECT_OF_CORRUPTION:
                m_uiJudgementEntry = SPELL_JUDGEMENT_OF_RIGHTEOUSNESS;
                m_uiSpell1Entry = 0;
            ...
            case NPC_ASPECT_OF_MALICE:
                m_uiJudgementEntry = SPELL_JUDGEMENT_OF_JUSTICE;
            ...
            case NPC_ASPECT_OF_SHADOW:
                m_uiJudgementEntry = SPELL_JUDGEMENT_OF_WISDOM;
    ...
    void SpellHit(Unit* pUnit, const SpellEntry* pSpellEntry)
    {
        error_log("Hit by spell Id: %u", pSpellEntry->Id);
        if (!m_bHit && pSpellEntry->Id == m_uiJudgementEntry && pUnit->GetTypeId() == TYPEID_PLAYER)
        {
            m_bHit = true;
            if (DoCastSpellIfCan(m_creature, SPELL_HOLY_BLAST, CAST_INTERRUPT_PREVIOUS | CAST_TRIGGERED) == CAST_OK)
                error_log("Judgement ID: %u casted", pSpellEntry->Id);
        }
    }
