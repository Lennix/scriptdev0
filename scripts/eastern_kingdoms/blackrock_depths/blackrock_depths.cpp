...
struct MANGOS_DLL_DECL npc_marshal_windsorAI : public npc_escortAI
{
    ...
    void WaypointReached(uint32 uiPointId)
    {
        ...
        switch(uiPointId)
        {
            ...
            case 36:
                DoSpawnGuards(470.87f, -9.29f, -69.81f, 1.86f, NULL, 3);
                break;
    ...
    bool DoCheckCellDoorOpenState(uint32 uiIdentifier)
    {
        if (!m_pInstance)
        {
            error_log("SD0: Blackrock Depths - Marshal Windsor - DoCheckCellDoorOpenState(): m_pInstance is NULL. Event can't continue!", uiIdentifier);
            return false;
        }
...
{
    npc_commander_gorshakAI(Creature* pCreature) : FollowerAI(pCreature) {Reset();}
    ...
    if (instance_blackrock_depths* m_pInstance = ((instance_blackrock_depths*)pGo->GetInstanceData()))
    {
        ...
        switch(uiRand)
        {
            case 0:
                ...
                pPlayer->SummonCreature(NPC_VORFALK, 902.425f, -140.241f, -49.756f, 0.0f, TEMPSUMMON_DEAD_DESPAWN, NULL);
                break;
            case 1:
                m_pInstance->DoRespawnGameObject(GO_DARK_KEEPER_NAMEPLATE2, 120*MINUTE);
                pPlayer->SummonCreature(NPC_BETHEK, 799.778f, -346.608f, -50.196f, 0.0f, TEMPSUMMON_DEAD_DESPAWN, NULL);
                break;
            case 2:
                m_pInstance->DoRespawnGameObject(GO_DARK_KEEPER_NAMEPLATE3, 120*MINUTE);
                pPlayer->SummonCreature(NPC_UGGEL, 962.34f, -396.09f, -60.83f, 0.0f, TEMPSUMMON_DEAD_DESPAWN, NULL);
                break;
            case 3:
                m_pInstance->DoRespawnGameObject(GO_DARK_KEEPER_NAMEPLATE4, 120*MINUTE);
                pPlayer->SummonCreature(NPC_ZIMREL, 596.518f, -232.087f, -38.356f, 0.0f, TEMPSUMMON_DEAD_DESPAWN, NULL);
                break;
            case 4:
                m_pInstance->DoRespawnGameObject(GO_DARK_KEEPER_NAMEPLATE5, 120*MINUTE);
                pPlayer->SummonCreature(NPC_OFGUT, 652.572f, 19.288f, -60.059f, 0.0f, TEMPSUMMON_DEAD_DESPAWN, NULL);
                break;
            case 5:
                // The Domicile
                m_pInstance->DoRespawnGameObject(GO_DARK_KEEPER_NAMEPLATE6, 120*MINUTE);
                pPlayer->SummonCreature(NPC_PELVER, 768.112f, -250.812f, -43.344f, 0.0f, TEMPSUMMON_DEAD_DESPAWN, NULL);
                break;
        }
    }
    return false;
...
{
    npc_ribbly_cronyAI(Creature* pCreature) : ScriptedAI(pCreature) {Reset();}
    ...
    if (instance_blackrock_depths* m_pInstance = (instance_blackrock_depths*)pGo->GetInstanceData())
    {
        if (m_pInstance->GetData(TYPE_BAR4) != DONE)
        {
            ...
            if (m_pInstance->GetData(TYPE_BAR4) == SPECIAL)
            {
                ...
                if (Creature* pHurley = pGo->SummonCreature(9537, 894.08f, -130.60f, -49.74f, 0.0f, TEMPSUMMON_DEAD_DESPAWN, NULL))
                    pHurley->AI()->AttackStart(pPlayer);
                if (Creature* pBodyGuard1 = pGo->SummonCreature(8901, 894.80f, -126.48f, -49.74f, 0.0f, TEMPSUMMON_DEAD_DESPAWN, NULL))
                    pBodyGuard1->AI()->AttackStart(pPlayer);
                if (Creature* pBodyGuard2 = pGo->SummonCreature(8901, 889.58f, -129.23f, -49.74f, 0.0f, TEMPSUMMON_DEAD_DESPAWN, NULL))
                    pBodyGuard2->AI()->AttackStart(pPlayer);
                if (Creature* pBodyGuard3 = pGo->SummonCreature(8901, 891.61f, -127.00f, -49.74f, 0.0f, TEMPSUMMON_DEAD_DESPAWN, NULL))
...
struct MANGOS_DLL_DECL npc_grimstoneAI : public npc_escortAI
{
        ...
        if (m_pInstance)
        {
            if (m_pInstance->GetData(TYPE_RING_OF_LAW) == SPECIAL)
            {
                ...
                for(uint8 i = 0; i < 4; ++i)
                {
                    ...
                    switch(uiRand[i])
                    {
                        case 0: uiCreatureEntry = NPC_LEFTY; break;
                        case 1: uiCreatureEntry = NPC_SNOKH_BLACKSPINE; break;
                        case 2: uiCreatureEntry = NPC_ROTFANG; break;
                        case 3: uiCreatureEntry = NPC_MALGEN_LONGSPEAR; break;
                        case 4: uiCreatureEntry = NPC_KORV; break;
                        case 5: uiCreatureEntry = NPC_REZZNIK; break;
                        case 6: uiCreatureEntry = NPC_VAJASHNI; break;
                        case 7: uiCreatureEntry = NPC_VOLIDA; break;
                    }
    ...
    void JustSummoned(Creature* pSummoned)
    {
        ...
        pSummoned->GetMotionMaster()->MovePoint(NULL, newPosX, newPosY, newPosZ);
    ...
    void WaypointReached(uint32 uiPointId)
    {
        switch(uiPointId)
        {
            ...
            case 4:
                switch (rand()%3)
				{
					case 0: DoScriptText(SAY_GRIMSTONE_4, m_creature); break;
					case 1: DoScriptText(SAY_GRIMSTONE_5, m_creature); break;
					case 2: DoScriptText(SAY_GRIMSTONE_6, m_creature); break;
				}
                CanWalk = false;
                EventTimer = 5000;
                break;
...
{
    npc_theldren_triggerAI(Creature* pCreature) : ScriptedAI(pCreature) {Reset();}
